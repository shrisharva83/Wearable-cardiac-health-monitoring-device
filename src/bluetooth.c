...
#include "wearyythmia_ble.h"
#include "wearyythmia_arrhythmia.h"
//UUID for rr interval service and characteristic
uint8_t rr_service_uuid[16]= {0xd0,0x36,0xba,0x8c,0xda,0xd1,0x4c,0xae,0xb8,0x7d,0x48,0x44,0x91,0x74,0x5c,0xcd};	
...
// structure to hold BLE data attributes
struct ble_data_att
{
	uint16_t pnn;
	uint8_t bat;
...
//structure for GATT characteristics value
...
        gl_profile_tab[PROFILE_C_APP_ID].service_id.id.uuid.len = ESP_UUID_LEN_128;
			...
			for(int i=0; i<16; i++){
				...
				gl_profile_tab[PROFILE_C_APP_ID].service_id.id.uuid.uuid128[i]=rr_service_uuid[i];
		}
        
...
void wearyythmia_ble_Init(void)
{
